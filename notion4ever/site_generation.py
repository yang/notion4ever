...
def verify_templates(config: dict):
    ...
    if Path(config["templates_dir"]).is_dir() and any(
        Path(config["templates_dir"]).iterdir()
    ):
...
def generate_archive(structured_notion: dict, config: dict):
    ...
    if config["build_locally"]:
        ...
        structured_notion["archive_url"] = str(
            (Path(config["output_dir"]).resolve() / archive_link)
        )
    else:
        archive_link = "Archive/index.html"
        structured_notion["archive_url"] = urljoin(
            structured_notion["base_url"], archive_link
        )
    ...
    for page_id, page in structured_notion["pages"].items():
        for field in ["date", "date_end", "last_edited_time"]:
            if field in page.keys():
                structured_notion["pages"][page_id][field] = dt_parser.isoparse(
                    page[field]
                )
...
def generate_page(page_id: str, structured_notion: dict, config: dict):
    ...
    page_url = page["url"]
    md_filename = (page["title"] or "ID_" + page_id) + ".md"

    if config["build_locally"]:
        folder = urljoin(page_url, ".")
        local_file_location = str(
            Path(folder).relative_to(Path(config["output_dir"]).resolve())
        )
    ...
    logging.debug(
        f"🤖 MD {Path(local_file_location) / md_filename}; HTML {Path(local_file_location) / html_filename}"
    )

    (config["output_dir"] / Path(local_file_location)).mkdir(
        parents=True, exist_ok=True
    )
    with open(
        (config["output_dir"] / Path(local_file_location) / md_filename).resolve(),
        "w+",
        encoding="utf-8",
    ) as f:
        metadata = (
    ...
    with open(
        (config["output_dir"] / Path(local_file_location) / html_filename).resolve(),
        "w+",
        encoding="utf-8",
    ) as f:
        # Specify template folder
        jinja_loader = jinja2.FileSystemLoader(config["templates_dir"])
        jtemplate = jinja2.Environment(loader=jinja_loader).from_string(tml)
        html_page = jtemplate.render(
            content=html_content, page=page, site=structured_notion
        )
        f.write(html_page)
